...
import { Cluster, OrgBillingOverview, PlanId, SupportTicket, TicketCategory, TicketSeverity } from '../../types/index';
import { Button } from '../common/UI';
import { SKYOPS_CONTACT_EMAIL, getSupportMailtoUrl } from '../../config/contact';
...
export const SupportManager: React.FC<SupportManagerProps> = ({ clusters = [] }) => {
  ...
  const currentPlanId: PlanId = overview?.subscription?.planId || 'FREE';
  ...
  const loadData = async () => {
    try {
      ...
      const [ticketsData, overviewData] = await Promise.all([
        ...
        api.getSubscriptionOverview().catch(() => null)
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`Support ticket dispatched to SkyOps Operations (${SKYOPS_CONTACT_EMAIL}).`);
      setShowModal(false);
      setSubject('');
      setDescription('');
  ...
  const supportTierInfo = {
    FREE: {
      name: 'Community Support',
      badgeClass: 'bg-zinc-800 text-zinc-300 border-zinc-700',
      description: 'Community-driven assistance and official email support via skyopsnetes2000@gmail.com.',
      slaText: 'Best effort / Community response'
    },
    PRO: {
      name: 'Standard Support',
      badgeClass: 'bg-sky-950/80 text-sky-300 border-sky-700/60',
      description: 'Standard technical assistance for engineering teams with verified ticketing and email support.',
      slaText: 'Standard email & ticketing support'
    },
    BUSINESS: {
      name: 'Priority Support',
      badgeClass: 'bg-emerald-950/80 text-emerald-300 border-emerald-700/60',
      description: 'Priority queue handling with accelerated resolution for mission-critical cluster incidents.',
      slaText: 'Priority queue with fast incident triage'
    },
    ENTERPRISE: {
      name: 'Dedicated Support & Custom SLA',
      badgeClass: 'bg-purple-950/80 text-purple-300 border-purple-700/60',
      description: 'Dedicated 24x7 SRE support engineer and custom contractual uptime SLAs.',
      slaText: '24x7 Dedicated SRE & contractual SLA'
    }
  }[currentPlanId];

  const emailSupportMailto = getSupportMailtoUrl(
    `[${supportTierInfo.name}] Support Request - ${currentOrg?.name || 'Workspace'}`,
    `Organization: ${currentOrg?.name || 'N/A'}\nOrganization ID: ${currentOrg?.id || 'N/A'}\nPlan: ${currentPlanId}\nUser: ${user?.email || 'N/A'}\n\nIssue Details:\n`
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header card with Plan Entitled Support Level */}
          ...
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              <h3 className="text-sm font-bold text-zinc-200 font-mono uppercase tracking-wider flex items-center gap-2">
                ...
                SkyOps Customer Support & Helpdesk
              </h3>
              <span className={`px-2.5 py-0.5 rounded-full border text-[11px] font-mono font-semibold ${supportTierInfo.badgeClass}`}>
                {supportTierInfo.name} ({currentPlanId} Plan)
              </span>
            </div>
            <p className="text-xs text-zinc-400 font-mono mt-1">
              {supportTierInfo.description}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 font-mono text-xs">
          <div className="p-4 bg-zinc-950 border border-zinc-800/80 rounded-xl space-y-2">
            ...
            <a
              ...
              className="text-sky-400 hover:text-sky-300 font-bold block text-sm break-all underline underline-offset-2"
            >
              {SKYOPS_CONTACT_EMAIL}
            </a>
            <p className="text-[11px] text-zinc-500">
              Primary inbox for billing, incident diagnostics, technical questions, and SLA inquiries.
            </p>
          </div>
          ...
          <div className="p-4 bg-zinc-950 border border-zinc-800/80 rounded-xl space-y-2">
            <div className="flex items-center gap-2 text-zinc-400 font-semibold">
              ...
              <span>Entitled SLA Level</span>
            </div>
            <div className="text-zinc-200 font-bold text-sm">
              {supportTierInfo.slaText}
            </div>
            <p className="text-[11px] text-zinc-500">
              Based on your active <strong className="text-zinc-400">{currentPlanId}</strong> tier subscription.
            </p>
          </div>

          <div className="p-4 bg-zinc-950 border border-zinc-800/80 rounded-xl space-y-2 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 text-zinc-400 font-semibold">
                <LifeBuoy className="w-4 h-4 text-purple-400" />
                <span>Direct Mail Dispatch</span>
              </div>
              <p className="text-[11px] text-zinc-400 mt-1">
                Prefill workspace metadata and dispatch directly via your email client.
              </p>
            </div>
            <a
              href={emailSupportMailto}
              className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded bg-zinc-900 hover:bg-zinc-800 text-sky-400 text-xs font-semibold border border-zinc-700 transition-colors"
            >
              <span>Email SkyOps Support</span>
              ...
              {tickets.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-zinc-500">
                    No support tickets submitted for this workspace yet. Contact {SKYOPS_CONTACT_EMAIL} or click Open Support Ticket above.
                  </td>
                </tr>
              ) : (
                tickets.map((t) => (
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <p className="text-[11px] text-zinc-400 font-mono">
              Your support inquiry will be routed to the SkyOps engineering team at <strong>{SKYOPS_CONTACT_EMAIL}</strong> under your <strong>{supportTierInfo.name}</strong> coverage.
            </p>
